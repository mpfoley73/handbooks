...
```{r}
p1 <- data.frame(x = seq(from = .05, to = 2.5, by = .01)) %>%
  mutate(`k = 0.5` = dweibull(x, .5),
         `k = 1` = dweibull(x, 1),
         `k = 1.5` = dweibull(x, 1.5),
         `k = 5` = dweibull(x, 5)) %>%
  pivot_longer(cols = -x) %>%
  ...
  theme(legend.position = "top") +
  labs(x = NULL, y = "PDF", color = NULL)

p2 <- data.frame(x = seq(from = 0, to = 2.5, by = .01)) %>%
  mutate(`k = 0.5` = pweibull(x, .5),
         `k = 1` = pweibull(x, 1),
         `k = 1.5` = pweibull(x, 1.5),
         `k = 5` = pweibull(x, 5)) %>%
  pivot_longer(cols = -x) %>%
  ggplot(aes(x = x, y = value, color = name)) + 
...
p1 + p2 +
  patchwork::plot_annotation(title = "Weibull")
```
