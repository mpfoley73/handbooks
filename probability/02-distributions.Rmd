...
> If $P$ is the probability of success in a random process then $P$ may be expressed as a random variable with a beta distribution $P \sim \mathrm{Beta}(\alpha, \beta)$. The probability of a probability of $P = p$ is
>
...
```{r fig.height=2.5}
data.frame(p = seq(0, 1, .01)) %>%
  mutate(Beta_5_5 = dbeta(p, 5, 5),
         Beta_10_10 = dbeta(p, 10, 10)) %>%
  pivot_longer(cols = -p) %>%
  ggplot(aes(x = p, y = value, color = name)) +
  geom_line() +
  labs(title = "Beta(5, 5) and Beta(10, 10) are symmetric.")
```

The distribution can shift to the left $\mathrm{Beta}(2, 5)$ or right $\mathrm{Beta}(5, 2)$.

```{r fig.height=2.5}
data.frame(p = seq(0, 1, .01)) %>%
  mutate(Beta_2_5 = dbeta(p, 2, 5),
         Beta_5_2 = dbeta(p, 5, 2)) %>%
  pivot_longer(cols = -p) %>%
  ggplot(aes(x = p, y = value, color = name)) +
  geom_line() +
  labs(title = "Beta(2, 5) and Beta(5, 2) are skewed.")
```
