...
The **Mann-Whitney U test**^[This test is sometimes called the Mann-Whitney test, Wilcoxon Rank Sum test, Wilcoxon-Mann-Whitney test, and the two-sample Wilcoxon test], is a nonparametric alternative to the independent-samples *t*-test. Use the Mann-Whitney U test when the samples are not normally distributed or when the response variables are ordinal rather continuous. In the first case where the normality assumption fails, the test evaluates H0 that the two samples are from the same population distribution. In the second case where the response variables are ordinal, the test evaluates the difference in medians. 

Mann-Whitney ranks the response values then calculates the mean rank for each group. The test statistic is the minimum of $U_1$ and $U_2$ where $U_1 = n_1 n_2 \frac{(n_1 (n_1 + 1))}{2} - R_1$ and $U_2 = n_1 n_2 \frac{(n_2 (n_2 + 1))}{2} - R_2$ and $R_1$ and $R_2$ are the rank sums. $U$ will equal 0 if there is complete separation between the groups, and $n_1 n_2$ if there is complete overlap. Reject H0 if $U$ is sufficiently small.
