...
Suppose a process follows an exponential probability distribution, $e^{\lambda t}$, but you do not know the parameter value, $\lambda$. You can estimate the value from the observations by comparing the actual outcomes to those produced by the range of possible $\lambda$'s and choosing the $\lambda$ the *maximizes* the likelihood of producing the observed outcome.

The likelihood the underlying exponential distribution parameter value is $\lambda$ given $n$ observations, is the product of the probability densities for each outcome,
