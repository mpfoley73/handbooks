...
If the data generating process produces discrete outcomes (counts), the sample count, $x$, as a random variable from a Poisson, binomial, normal, or multinomial distribution, or as a random variable from a theoretical outcome.
...
#### Example {-}
Poisson sampling is used to model counts of events that occur randomly over a *fixed period of time*. Here is a case study. A researcher is analyzing World Cup play using a sample consisting of the `r sum(dat_pois$freq)` matches in the first round of the 2002 World Cup. The average match produced a mean/sd of `r weighted.mean(x = dat_pois$goals, w = dat_pois$freq) %>% scales::comma(accuracy = .01)` $\pm$ `r radiant.data::weighted.sd(x = dat_pois$goals, w = dat_pois$freq) %>% scales::comma(accuracy = .01)` goals, lower than the 1.5 historical average during World Cup play. Should you reject the null hypothesis that the sample is representative of the overall population?
...
You might also check whether the data is consistent with a Poisson model.
