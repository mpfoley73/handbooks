...
Let $T^*$ be a random variable representing the time until the event, and $U$ be a random variable representing the time until censoring (right-censoring). The observed value, $T$, is whichever comes first, $T = \mathrm{min}(T^*, U)$. The `lung` data set is typical, with variable `time` equaling $T$ and `status` equaling $\delta = I[T^* < U]$ where 1 = No (i.e., censored), and 2 = Yes (i.e., event occurred).
...
The functions are related by the multiplication rule, $P(AB) = P(A|B)P(B)$. The event probability at $t$, $f(t) = F'(t)$, is the conditional event probability of the event ocurring at $t$ given survival up to $t$ multiplied by the probability of survival up to $t$.
...
where $n_i$ is the number of subjects at risk at time $i$ and $d_i$ is the number incurring the event. The KM curve falls only when an event occurs, not when a subject is censored. Confidence limits are calculated using the "delta" method to obtain the variance of $\log \hat{S}(t)$.

Calculate $\hat{S}(t)$ with `survival::survfit()`. Data set `lung` records the `status` (1 = censored, 2 = dead) of 228 lung cancer patients. `survfit()` operates on a `Surv` object, created by `Surv()`. Event indicators can be coded as 0|1 or 1|2.^[See neat discussion in Note section of `Surv()` help file.] Explanatory variables can be defined as factors, but the documentation for `Surv()` explains that it treats factor *event* variables as multiple endpoint data, so leave `status` numeric.
...
112 of 138 males and 53 of 90 females died. The median survival time was `r km_fit %>% summary %>% pluck("table") %>% .["sex=Male", "median"]` days for males and `r km_fit %>% summary %>% pluck("table") %>% .["sex=Female", "median"]` days for females. The 95% CI is defined around the median time to event. Retrieve these values from the `summary()$table` object. E.g., `summary(km_fit)$table[1, "events"]` = `r summary(km_fit)$table[1, "events"]`.

`broom::tidy()` summarizes the data by recorded time, returning the number at risk, the number of events, and the number censored. At time *t* = 11, 3 of the 138 males at risk died, so the KM estimate of survival at *t* = 11 is $S(11) = 1 - \frac{3}{138} = .978$. At *t* = 12, 1 of the 135 that remained died, so $S(12) = S(11) \cdot \frac{1}{135} = .971$, and so on.

```{r}
km_fit %>% broom::tidy()
```

Don't yield to the temptation to construct your own ggplot - **survminer** does a good job plotting KM models. Vertical drops indicate events and vertical ticks indicate censoring.
...
The log-rank test statistic is the sum of the differences between the observed and expected failures, $U_0 = \sum (d_{Ai} - e_{Ai})$, normalized by dividing by the square-root of its variance, $V_0 = Var({U_0}) = \sum v_{Ai}$.
...
The *p*-value for $\chi_1^2 = 10.3$ is 0.001, so reject $H_0$ that males and females have identical survival patterns.
...
Find the value of $\psi$ that maximizes $L(\psi)$. $L(\psi)$ is difficult to optimize, but its log is easier. $l(\beta) = X \beta - \log (3 + 3 e^{X\beta}) - \log (1 + 3 e^{X\beta}) - \log (1 + 2 e^{X\beta})$. A function will search for the $\beta$ producing the global max. 
...
The Cox proportional hazards model is analogous to the [logistic regression](https://bookdown.org/mpfoley1973/supervised-ml/generalized-linear-models-glm.html#logistic-regression) model. Rearranging,

$$\ln \left[ \frac{h(t)}{h_0(t)} \right] = X \beta$$

Whereas logistic regression predicts the *log odds* of the response, Cox regression predicts the *log relative hazard* (relative to the unspecified baseline). $\beta$ is the change in the log of the relative hazard associated with a one unit change in $X$. Its antilog is the hazard ratio (HR). A positive $e^{\beta_j}$ means the hazard increases with the covariate. 
...
Fit a Cox proportional hazards model with `coxph()`. Returning to the `lung` data set,
