...
The KM analysis is valid under certain conditions.

* The event status consists of two mutually exclusive and collectively exhaustive states: "censored" or "event".
* The time to an event or censorship should be clearly defined and precisely measured.
* Left-censoring (unknown starting point) should be minimized or avoided.
* Censoring event causes should be independent of the event.
* There are no secular trends (e.g., staggered starting times and introduction of new therapies).
* The amount and pattern of censorship should be similar per group.

The first four assumptions relate to study design and cannot be tested, but the secular trends assumption can be tested by running multiple Kaplan-Meier tests for different time intervals of the study, and the censorship pattern assumption can also be tested.

## Log-Rank Test

It's not immediately obvious how to compare two survival distributions, since they can cross, diverge, etc. When observations do not follow a defined parametric distribution, use a non-parametric test. In this case, the log-rank test is the solution. The alternative hypothesis, termed the Lehmann alternative, is that one survival distribution is uniformly higher than the other, $H_A : S_1(t) = [S_0(t)]^\psi$. Equivalently, the hazard functions are proportional, $h_1(t) = \psi h_0(t)$. $H_0 : \psi = 1$.
...
The Cox proportional hazards model expresses the expected hazard at time t, $h(t)$, as a baseline hazard, $h_0(t)$, multiplying the exponential of a linear combination of parameters.

$$h(t) = h_0(t) \cdot e^{X \beta}$$

The model is analogous to the [logistic regression](https://bookdown.org/mpfoley1973/supervised-ml/generalized-linear-models-glm.html#logistic-regression) model. Rearranging,

$$\ln \left[ \frac{h(t)}{h_0(t)} \right] = X \beta$$

Whereas logistic regression predicts the *log odds* of the response, Cox regression predicts the *log relative hazard* (relative to the unspecified baseline). The model is estimated by the process of partial likelihood.

$\beta$ is the change in the log relative hazard associated with a one unit change in $X$. Its antilog is the hazard ratio (HR). A positive $e^{\beta_j}$ means the hazard increases with the covariate. 
...
Even though the baseline hazard is unspecified, the Cox model can still be estimated by the method of partial likelihood. Consider the first failure time, $t_1$. The probability that subject $i$ is the one to fail is the ratio of $i$'s hazard and the sum of hazards for all $R_1$ subjects at risk,

$$p_1 = \frac{h_i(t_1)}{\sum_{k \in R_1} h_k(t_1)} = \frac{h_0(t_1) \psi_i}{\sum_{k \in R_1} h_0(t_1) \psi_k} = \frac{\psi_i}{\sum_{k \in R_1} \psi_k}$$

The next failure event has a reduced $R_2$ subjects at risk. The partial likelihood for $D$ failure times is the product $L_i = p_1 p_2 \cdots p_D$. The Cox model uses maximum partial likelihood estimation to find the value of $\psi$ that maximizes the likelihood function.
...
The participant censoring plot shows censored cases are equally spread over time and not too dissimilar for the *asymptomatic* and *ambulatory* groups, but the *bedridden* group had a low number of censoring events.
