...
(*notes from [here](https://data.princeton.edu/wws509/notes/c7s6).*)

Cox proposed an extension to the proportional hazards model to discrete times using logistic regression:

$$
\frac{h(t;x)}{1 - h(t;x)} = \frac{h_0(t)}{1 - h_0(t)} e^{xb}
$$

where $h_0(t)$ is the baseline hazard at $t$ and $e^{xb}$ is the relative risk associated with the covariate values $x$. The log of each side is the logit of the hazard,

$$
\mathrm{logit} \{h(t;x)\} = \mathrm{logit \{h_0(t)\}} + xb
$$

The model treats time as a discrete factor by introducing a parameter $\alpha = \mathrm{logit \{h_0(t)\}}$ for each possible event time, $t$. Parameter interpretation is the same as other logistic regression. 

An alternative extension to the proportional hazards model starts from the survival function
