...
Sometimes you will see the _cost function_ optimized. This is the negative of of the log likelihood function.

### Case Study

Dataset `donner` contains _n_ = 45 observations of survival status (`surv`) of the members of the Donner party with explanatory variables `age` and `sex`.
...
m <- glm(surv ~ sex*age, 
         data = donner_centered, 
         family = binomial(link = logit))
summary(m)
```

The "z value" and "Pr(>|z|)" statistics follow directly from the "Estimate" and "Std. Error". The "z value" is the Wald $z$ statistic, $z = \hat{\beta} / SE(\hat{\beta})$. Its square, $z^2$, is the Wald chi-squared statistic. The _p_-value is the area to the right of $z^2$ in the $\chi_1^2$ density curve:
...
Plug in values to interpret the model. The log odds of a 24 year-old female surviving is $\hat{y}$ = `r predict(m, newdata = data.frame(sex = "F", age = 24)) %>% scales::number(accuracy = .01)`.  The log odds of a 24 year-old male surviving is $\hat{y}$ = `r predict(m, newdata = data.frame(sex = "M", age = 24)) %>% scales::number(accuracy = .01)`.
...
Log odds are not easy to interpret. Exponentiate the log odds to get the **odds**.
...
odds_f_18 <- exp(coef(m)["(Intercept)"] + coef(m)["age"] * (-10))
odds_m_28 <- exp(coef(m)["(Intercept)"] + coef(m)["sexM"])
odds_m_18 <- exp(coef(m)["(Intercept)"] + coef(m)["sexM"] + coef(m)["age"] * (-10) + coef(m)["sexM:age"]*(-10))

prob_f_28 <- odds_f_28 / (1 + odds_f_28)
prob_f_18 <- odds_f_18 / (1 + odds_f_18)
prob_m_28 <- odds_m_28 / (1 + odds_m_28)
prob_m_18 <- odds_m_18 / (1 + odds_m_18)

odds_f_28_fmt <- scales::number(odds_f_28, accuracy = .1)
odds_f_18_fmt <- scales::number(odds_f_18, accuracy = .1)
odds_m_28_fmt <- scales::number(odds_m_28, accuracy = .1)
odds_m_18_fmt <- scales::number(odds_m_18, accuracy = .1)
```

* The survival odds of a `r median(donner$age)` year old female are $\hat{y} = e^{(Intercept)}$ = `r odds_f_28_fmt`. The expected probability of surviving is `r glue::glue("{odds_f_28_fmt} / (1 + {odds_f_28_fmt}) = {scales::number(prob_f_28 * 100, accuracy = 1)}")`%.
* The survival odds of a `r median(donner$age) - 10` female are $\hat{y} = e^{(Intercept) -10 \cdot \mathrm{age}}$ = `r odds_f_18_fmt`. The expected probability of surviving is `r glue::glue("{odds_f_18_fmt} / (1 + {odds_f_18_fmt}) = {scales::number(prob_f_18 * 100, accuracy = 1)}")`%.
* The survival odds of a `r median(donner$age)` male are $\hat{y} = e^{(Intercept) + sexM}$ = `r odds_m_28_fmt`. The expected probability of surviving is `r glue::glue("{odds_m_28_fmt} / (1 + {odds_m_28_fmt}) = {scales::number(prob_m_28 * 100, accuracy = 1)}")`%.
* The survival odds of a `r median(donner$age) - 10` male are $\hat{y} = e^{(Intercept) -10 \cdot \mathrm{age} -10 \cdot \mathrm{sex} -10 \cdot \mathrm{sexM:age}}$ = `r odds_m_18_fmt`. The expected probability of surviving is `r glue::glue("{odds_m_18_fmt} / (1 + {odds_m_18_fmt}) = {scales::number(prob_m_18 * 100, accuracy = 1)}")`%.
