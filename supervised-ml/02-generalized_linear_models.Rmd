...
Jump right in and fit the model.
...
Above Fisher, the null deviance is the likelihood ratio of the intercept-only model, $G^2$ = `r scales::number(summary(cs1$fit)$null.deviance, accuracy = .01)`. The residual deviance, is the likelihood ratio $G^2$ = `r scales::number(summary(cs1$fit)$deviance, accuracy = .01)` after including the model covariates. It is the sum of the squared deviance residuals discussed above. The residual deviance is distributed chi-squared and can be used to test whether the model differs from the saturated model (model with as many coefficients as observations, $G^2$ = 0, *df* = 0) where $H_0$ = no difference. The deviance test for lack of fit fails to reject the null hypothesis.
...
Start your interpretation with the dependent variable. Plug in values for the predictor variables to get predictions. $\hat{y}$ is the log odds of a man of average age, height, and aerobic capacity having heart disease.
...
Exponentiate to get the **odds**, $\mathrm{odds}(\hat{y}) = \exp (\hat{y}) = \frac{\pi}{1 - \pi}$.

```{r}
(my_odds <- exp(my_log_odds))
```

Solve for $\pi = \frac{\exp (\hat{y})}{1 + \exp (\hat{y})}$ to get the **probability**. You can do the math, or use `predict(type = "response")`.
...
I may already be dead. Move on to the coefficients. Interpret the coefficient estimates as the change in the log odds of $y$ due to a $\delta = x_a - x_b$ change in $x$. You might say $\beta$ is the log odds ratio of a one unit change in $x$.
...
For example, an age increase of one year increases the log-odds of heart disease by a factor of `r coef(cs1$fit)["age_c"] %>% comma(.001)`. Exponentiate this to get the increase in the odds of heart disease, `r coef(cs1$fit)["age_c"] %>% exp() %>% comma(.001)`. It might be more useful to calculate the effect of a *decade* increase in age. Multiply the coefficient by 10 before exponentiating, or raise the exponentiated coeficient by 10. The effect of a 10-year increase in age is to increase the odds of heart disease by `exp(coef(cs1$fit)["age_c"] * 10)` = `r exp(coef(cs1$fit)["age_c"] * 10) %>% comma(.001)`. The odds more than double every ten years.

The reference case in the model is `gender` = "Female", `age` centered at `r comma(cs1$centers$age, .1)` (`age_c` = 0), `weight` centered at `r comma(cs1$centers$weight, .1)` (`weight_c` = 0), and `VO2max` centered at `r comma(cs1$centers$VO2max, .1)` (`VO2max_c` = 0).

```{r}
# Set newdata to the reference case
newdata <- list(age_c = 0, weight_c = 0, VO2max_c = 0, gender = "Female")
```

The intercept term is the log-odds of heart disease at the reference case. Because the data is centered, the reference case is actually meaningful (i.e., not age 0, weight 0, and VO2max 0).

```{r collapse=TRUE}
# Predicted value is the log-odds of heart disease
(log_odds <- coef(cs1$fit)["(Intercept)"])
(pred <- predict(cs1$fit, newdata = newdata, type = "link"))
...
# Probability.
(prob <- odds / (1 + odds))
(pred <- predict(cs1$fit, newdata = newdata, type = "response"))
```

The log odds of heart disease for a `r comma(cs1$centers$age, .1)` year old female weighing `r comma(cs1$centers$weight, .1)` kg and with `r comma(cs1$centers$VO2max, .1)` VO2max are $\hat{y} = \mathrm{Intercept}$ = `r comma(log_odds, .01)`. The odds are $\exp(\hat{y})$ = `r comma(odds, .01)`. The expected probability is $\frac{\exp(\hat{y})}{1 + \exp(\hat{y})}$ = `r percent(prob, .1)`. What if they are male instead of female? 

```{r include=FALSE}
(log_odds_m <- coef(cs1$fit)["(Intercept)"] + coef(cs1$fit)["genderMale"])
(odds_m <- exp(log_odds_m))
(prob_m <- odds_m / (1 + odds_m))
```

The coefficient for gender is the log odds-ratio, `coef(cs1$fit)["genderMale"]` = `r comma(coef(cs1$fit)["genderMale"], .01)`. Its exponential is the odds ratio, `exp(coef(cs1$fit)["genderMale"])` = `r comma(exp(coef(cs1$fit)["genderMale"]), .01)`. That is, a male's odds of heart disease are `r comma(exp(coef(cs1$fit)["genderMale"]), .01)` times that of a female's, ceteris paribus. The reference level male's odds of heart disease are $\exp(\hat{y})$ = `r comma(odds_m, .01)` (probability = `r percent(prob_m, .1)`). Indeed, the ratio of the male's odds to the female's odds is `r comma(odds_m, .01)` / `r comma(odds, .01)` = `r comma(odds_m / odds, .01)`.

A one-unit increase in any of the continuous independent variables is interpreted similarly. The reference level is zero, or just the value before the increase. The coefficient for age is the log odds-ratio, `coef(cs1$fit)["age_c"]` = `r comma(coef(cs1$fit)["age_c"], .01)`. Its exponential is the odds ratio, `exp(coef(cs1$fit)["age_c"])` = `r comma(exp(coef(cs1$fit)["age_c"]), .01)`. That is, a person's odds of heart disease increase by a factor of `r comma(exp(coef(cs1$fit)["age_c"]), .01)` per year of life. The impact of 10 years of life would be to multiply the odds by `exp(coef(cs1$fit)["age_c"]*10)` = `r comma(exp(coef(cs1$fit)["age_c"]*10), .01)` (notice that the 10 multiplies the coefficient, then their _product_ is exponentiated).

`oddsratio::or_glm()` calculates the odds ratio from a specified increment in the predictor values.
...
```{r}
cs1$dat %>%
  glm(heart_disease ~ age + weight + VO2max, data = ., family = "binomial") %>%
  augment() %>%
...
(I don't know why this doesn't work. It _would_ work if the underlying model is OLS instead of GLM. The answer seems to be related to GVIF vs VIF, but I didn't figure it out.)

```{r include=FALSE}
tmp <- augment(cs1$fit, type.predict = "response") %>% filter(abs(.std.resid) >= 2) %>% pull(.std.resid)
```

Check for influential outliers by predicting the response probabilities and filtering for the predictions more than two standard deviations from the actual value and a Cook's Distance greater than 4/N = `r 4 / nrow(cs1$dat)`.^[[This article](https://towardsdatascience.com/assumptions-of-logistic-regression-clearly-explained-44d85a22b290) suggests using three standard deviations for an outliers threshold.] Recall that Cook's distance measures how much predicted values change when observation *i* is removed from the data. Only two fitted values were both an outlier and influential, row ids 59 and 70. An index plot of Cook's Distance shows the two points at the far left. You might examine the observations for validity. Otherwise, proceed and explain that there were two standardized residuals with value of `r comma(tmp[1], .01)` and `r comma(tmp[2], .01)` standard deviations which were kept in the analysis. 
...
There are several ways to evaluate the model. 
...
The fitted model is significant, $\chi^2$(`r abs(cs1$lrtest$Df[2])`) = `r comma(cs1$lrtest$Chisq[2], .1)`, *p* < .001.

One is the pseudo-R-squared measures^[Nice write-up [here](https://stats.oarc.ucla.edu/other/mult-pkg/faq/general/faq-what-are-pseudo-r-squareds/). Sounds like you should use pseudo R2 for model comparison, not for reporting goodness of fit]. A second is accuracy and ROC.

```{r}
(cs1$pseudo_r2 <- DescTools::PseudoR2(cs1$fit, which = c("CoxSnell", "Nagelkerke", "McFadden")))
```

Laerd interprets this as the model explained `r percent(cs1$pseudo_r2["Nagelkerke"], .1)` (Nagelkerke R2) of the variance in heart disease. This is how your would interpret R2 in an OLS model. [UCLA](https://stats.oarc.ucla.edu/other/mult-pkg/faq/general/faq-what-are-pseudo-r-squareds/) points out that the various pseudo R-squareds measure other aspects of the model and are unique to the measured quantity. A pseudo R-squared is not very informative on its own; its value is in comparison of models predicting the same outcome. The cross-tabulation of observed and predicted classes is the better way to go.
...
```{r}
cs1$confusion_matrix$byClass
```

Finally, plot the [gain curve or ROC curve](https://community.tibco.com/wiki/gains-vs-roc-curves-do-you-understand-difference). 
