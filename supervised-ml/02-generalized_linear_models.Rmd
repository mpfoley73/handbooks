...
augment(cs1$fit, type.predict = "response") %>%
  ...
  geom_hline(yintercept = 4 / nrow(cs1$dat), linetype = 2, color = "firebrick") + 
...
Variance inflation factors (VIF) measure the degree of multicollinearity in a set of independent variables. VIF is the ratio of the overall model variance to the variance of a model with just the predictor in question. VIFs can range from $[1, \infty]$. A VIF over 5 is problematic. These are excellent.
...
You have already fit the model by this point. Take a look at the model summary.

```{r}
summary(cs1$fit)
```

Plug in values to get a more intuitive understanding. The log odds of a 50 year-old man weighing 165 pounds and average VO2max having heart disease is $\hat{y}$ = `r predict(cs1$fit, newdata = list(age = 50, weight = 165, VO2max = mean(cs1$dat$VO2max), gender = "Male")) %>% scales::number(accuracy = .01)`. If he was 60 years old, they would be $\hat{y}$ = `r predict(cs1$fit, newdata = list(age = 60, weight = 165, VO2max = mean(cs1$dat$VO2max), gender = "Male")) %>% scales::number(accuracy = .01)`.

```{r collapse=TRUE}
new_dat <- expand.grid(gender = "Male", age = c(2:8)*10, VO2max = mean(cs1$dat$VO2max), weight = 165)
predict(cs1$fit, newdata = new_dat)
```
...
```{r collapse=TRUE}
predict(cs1$fit, newdata = new_dat) %>% exp()
```
...
```{r collapse=TRUE}
predict(cs1$fit, newdata = new_dat, type = "response")
```

I may already be dead. Interpret the coefficient estimates as the change in the log odds of $y$ due to a $\delta = x_1 - x_0$ unit change in $x$.
...
For example, an age increase of one year increases the log-odds of heart disease by a factor of `r coef(cs1$fit)["age"] %>% comma(.001)`. Exponentiate this to get the increase in the odds of heart disease, `r coef(cs1$fit)["age"] %>% exp() %>% comma(.001)`. It might be more useful to calculate the effect of a *decade* increase in age. Multiply the coefficient by 10 before exponentiating, or raise the exponentiated coeficient by 10. The effect of a 10-year increase in age is to increase the odds of heart disease by `exp(coef(cs1$fit)["age"] * 10)` = `r exp(coef(cs1$fit)["age"] * 10) %>% comma(.001)`. The odds more than double every ten years.

The reference case is `sex` = "F" and `age` = 0. 
...
```{r}
oddsratio::or_glm(cs1$dat, cs1$fit, incr = list(age = 10, weight = 10, VO2max = 5))
```
...
The null deviance is the likelihood ratio of the intercept-only model, $G^2$ = `r scales::number(summary(cs1$fit)$null.deviance, accuracy = .01)`. The residual deviance is the likelihood ratio $G^2$ = `r scales::number(summary(cs1$fit)$deviance, accuracy = .01)` after including the model covariates. $G^2$ is large, so reject the null hypothesis of no age or sex effects. An ANOVA table shows the change in deviance from successively adding each variable to the model.
